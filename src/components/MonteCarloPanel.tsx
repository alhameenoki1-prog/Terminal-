...
// GBM + optional Student-t fat tails (df=5)
// Regime-conditional sigma adjustment (Risk-On: ×0.85, Transition: ×1.0, Risk-Off: ×1.2, Crisis: ×1.5)
...
// Student-t variate via ratio of normal to chi (df=5 for fat tails)
function studentTRandom(df = 5): number {
  const z  = gaussianRandom()
  // Chi-squared with df degrees of freedom as sum of squared normals
  let chiSq = 0
  for (let i = 0; i < df; i++) {
...
function runMonteCarlo(
  ...
  const cvar95   = finals.slice(0, var95idx).reduce((a, b) => a + b, 0) / Math.max(1, var95idx)
  const cvar99   = finals.slice(0, var99idx).reduce((a, b) => a + b, 0) / Math.max(1, var99idx)
...
export function MonteCarloPanel() {
  ...
  const [regimeAdj,  setRegimeAdj]  = useState(false)
  ...
  const run = useCallback(() => {
    ...
    setRunning(true)
    setTimeout(() => {
      let sigma = parseFloat(annualVol) || 0.6
      // Regime-conditional sigma adjustment (Section 2.6)
      if (regimeAdj) {
        const mult = REGIME_VOL_MULT[regime.regime] ?? 1.0
        sigma = sigma * mult
      }
      ...
      const r  = runMonteCarlo(currentPrice, mu, sigma, horizon, e, t, s, useStudentT, 5000)
      setResult(r)
      setRunning(false)
    }, 50)
  }, [currentPrice, annualVol, horizon, entry, target, stop, useStudentT, regimeAdj, regime.regime])

  const pct = (v: number) => `${(v * 100).toFixed(1)}%`

  // Effective sigma for display
  const displaySigma = regimeAdj
    ? ((parseFloat(annualVol) || 0.6) * (REGIME_VOL_MULT[regime.regime] ?? 1.0)).toFixed(3)
    : (parseFloat(annualVol) || 0.6).toFixed(3)
  ...
  return (
    <div className="p-3 flex flex-col gap-3">
      ...
      <div className="grid grid-cols-2 gap-2">
          ...
          >
            {watchlist.map((w) => (
              <option key={w.symbol} value={w.symbol}>
                {w.symbol.replace('USDT','').replace('=X','').replace('^','')}
              </option>
            ))}
          </select>
        </div>
        <div>
          <label className="font-mono text-2xs text-terminal-faint block mb-1">
            Annual Vol {regimeAdj && `(adj: ×${REGIME_VOL_MULT[regime.regime]})`}
          </label>
          <input
            type="number"
            step="0.05"
              ...
              <button
                ...
                className={`px-1.5 py-0.5 font-mono text-2xs rounded transition-colors ${
                  horizon === h.days
                    ...
                    : 'bg-terminal-panel text-terminal-faint'
                }`}
              >
                {h.label}
              </button>
      ...
      {/* Distribution + regime toggles */}
      <div className="flex gap-3 flex-wrap">
        <label className="flex items-center gap-1.5 cursor-pointer">
          <input
            type="checkbox"
            checked={useStudentT}
            onChange={(e) => setUseStudentT(e.target.checked)}
            className="accent-terminal-accent"
          />
          <span className="font-mono text-2xs text-terminal-faint">
            Student-t (df=5) fat tails
          </span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer">
          <input
            type="checkbox"
            checked={regimeAdj}
            onChange={(e) => setRegimeAdj(e.target.checked)}
            className="accent-terminal-accent"
          />
          <span className="font-mono text-2xs text-terminal-faint">
            Regime σ adj ({regime.regime})
          </span>
        </label>
      </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="bg-terminal-panel rounded p-2 border border-terminal-border/50">
            <div className="font-mono text-2xs text-terminal-faint mb-2">
              Price Distribution at {horizon}D
              {useStudentT && <span className="ml-1 text-terminal-accent">[t-dist]</span>}
              {regimeAdj && <span className="ml-1 text-yellow-400">[{regime.regime}]</span>}
            </div>
          ...
          {useStudentT && (
            <div className="font-mono text-2xs text-terminal-accent/80 bg-terminal-panel rounded p-2 border border-terminal-accent/20">
              Student-t (df=5): fat tails active — VaR/CVaR will be larger than Gaussian equivalent
            </div>
          )}
        </div>
      )}
